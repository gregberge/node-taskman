...
Worker.prototype.start = function (callback) {
  this.initialize(_.bind(function () {
    var next = _.compose(_.partial(process.nextTick, _.bind(this.process, this)), callback || _.noop);
...
Worker.prototype.pause = function (callback) {
  this.initialize(_.bind(this.setStatus, this, Worker.STATUS_PAUSED, callback));
};
...
Worker.prototype.stop = function (callback) {
  this.initialize(_.bind(this.setEndDate, this, new Date(), callback));
};
...
Worker.prototype.process = function () {
  ...
  this.getInfos(_.bind(function (err, infos) {
    if (! infos.end_date || Date.now() > new Date(infos.end_date).getTime()) {
    ...
    if(infos.status === Worker.STATUS_PAUSED) {
      ...
      next = _.partial(setTimeout, _.bind(this.process, this), pauseTime);
      Worker.prototype.setEndDate(new Date(Date.now() + pauseTime).toJSON(), next);
      return ;
    }
    ...
    this.queue[typeFunction](infos.data_per_tick, _.bind(function (err, result) {
      ...
      if (! result[0]) {
        next = _.partial(setTimeout, _.bind(this.process, this), infos.waiting_timeout * 1000);
        this.setStatus(Worker.STATUS_WAITING, next);
        return ;
      }
      ...
      var callback = _.bind(function () {
        ...
        next = _.partial(setTimeout, _.bind(this.process, this), infos.loop_sleep * 1000);
