...
Worker.prototype.initialize = function (callback) {
  ...
  async.series([
    ...
    this.setUnique.bind(this, !! this.queue.options.unique),
...
Worker.prototype.stop = function (callback) {
  this.initialize(this.setEndDate.bind(this, (new Date()).toJSON(), callback));
};
...
Worker.prototype.pop = function (err, result, infos) {
  ...
  next = function () {
    var ctrl = setTimeout(callback.bind(null, new Error('Timeout')), this.options.timeout);

    try {
      this.action(result, function launch () {
        clearTimeout(ctrl);
        callback();
      }, this);
    } catch (err) {
 ...
 * @param function callback Callback
 */
Worker.prototype.setUnique = function (isUnique, callback) {
  this.driver.hset(this.completeName, 'unique', isUnique, callback);
};
