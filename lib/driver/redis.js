...
RedisDriver.prototype.atomicUniquePop = function (queue, type, callback) {
  async.waterfall([
    this.llen.bind(this, queue),
    function (count, callback) {
      this.client.lindex(
        this.getCompleteQueueName(queue),
        type === 'rpop' && count ?  --count : 0,
        callback);
    }.bind(this),
    function (data, callback) {
      ...
      multi.hdel(this.getUniqueSetName(queue), this.shasum(data));
      multi.exec(callback);
    }.bind(this)
  ], function (err, replies) {
    if (err) return callback(err);
    callback(null, _.first(replies) || null);
  });
};
...
RedisDriver.prototype.exists = function (queue, data, callback) {
  this.client.hset(this.getUniqueSetName(queue), this.shasum(data), data, function (err, count) {
    if (err) return callback(err);
